...
A common task when working with futures is transforming or traversing a collection that will call a method that
returns a future. The standard methods provided can, with enough nesting, leads to difficult to read code:
...
== Limiting concurrent parallel Transforming and traversing collections using workers
