...
== Why?
Scala Futures are awesome! But...

* Lots of boiler plate needed to create parallel futures
* Future.sequence doesn't always fail immediately
* Future.sequence only returns the first exception (and throws away the rest!)
* Waiting on Futures requires unnecessary cruft (very common in testing code!)
* Missing many common operations such as fold, flatten, etc
* Missing common scheduling operations such as delayed futures and periodically repeating tasks
...
* +Future.fold+ - fold Future[A] Success[A]/Failure[A] to Future[B]
* +Collection.sequence+ - Future.sequence with less typing
...
* +Future.sideEffect+ - do something after a Future completes. Don't change the return type but fail the Future if the
side effect fails
* +Future.delayed+ - start a Future after a delay
* +ScheduledExecutionContext.scheduleCancellable+ - start a Future after a delay that can be cancelled
* +ScheduledExecutionContext.scheduleAtFixed+ - create a repeating task
