...
* Adds new concurrent flow control primitives +concurrently+, +serially+ and +workers+ for performing collection operations such as +map+, +flatMap+, +foreach+ when the function returns a Future of the result (i.e. A => Future[B]) instead of the result (i.e. A => B)
