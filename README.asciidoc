...
serial and parallel execution flow control primitives for working with code
that uses +scala.concurrent.Future+.

* Adds concurrent flow control primitives +async+ and +async.par+ for
performing heterogeneous and homogeneous asynchronous tasks.
+
NOTE: An asynchronous task consists of two or more calls to function(s) that
return a Future result (A => Future[B]) instead of the result (A => B)
+
...
Merge' section below)
** Ensure all exceptions are thrown during asynchronous tasks are thrown
(instead of only the first)
** Return a future of the new collection or tuple, e.g. given +f: Int =>
Future[String]+:
