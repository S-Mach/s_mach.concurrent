...
* Future.sequence doesn't always fail immediately (a quick failure of a single Future might have to wait for all other Futures to complete before failing the output Future!)
...
* No methods for controlling how multiple Futures execute
...
* +ScheduledExecutionContext.scheduleAtFixed+ - create a repeating task that can paused or cancelled
