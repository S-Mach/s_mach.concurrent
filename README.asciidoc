...
* Wish there were methods for controlling how multiple Futures execute (no retry, no throttle, no parallel control)
* Hate all of the boilerplate needed to create multiple futures that run in parallel
...
* Annoyed that you have to use Java's ScheduledExecutorService for delayed or periodically repeating tasks (or an ActorSystem)
