...
* Adds retry and throttle control for collection concurrent operations
...
.Example 8: Adding retry and throttle Control to collection concurrent operations
[source,scala,numbered]
----
val oomItemIdBatch = (1 to 10).toList.map(_.toString).grouped(2).toList
val future = { // necessary for pasting into repl
  for {
    oomItem <- {
      ...
      oomItemIdBatch
        .workers(2)
        .throttle(100.millis) // limit to 1 batch per 100 milliseconds
        .flatMap { batch =>
          batch.
            .concurrently
            .retry { // retry certain exceptions
              case _: TimeoutException :: tail if tail.size < 3 => true
              case _: SocketTimeoutException :: _ if tail.size < 3 => true
              case _ => false
            }
            .throttle(1.millis) // limit to 1 id per 1 millisecond
            .map(read)
        }
    }
    _ = println("Computing...")
