...
* Adds new concurrent flow control primitives +concurrently+, +serially+ and +workers+ for performing collection operations such as +map+, +flatMap+ and +foreach+ when the called function returns a Future of the result (i.e. A => Future[B]) instead of the result (i.e. A => B)
* Allows enabling optional progress reporting, failure retry and/or throttle control for all collection concurrent operations
* Adds +ScheduledExecutionContext+, a wrapper for +java.util.concurrent.ScheduledExecutorService+ that provides a Scala interface for scheduling delayed and periodic tasks
