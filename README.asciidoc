...
== Pre-requisites & adding to build.sbt
1. +s_mach.concurrent+ works with Scala 2.9.3, 2.10.4 or 2.11.1
...
+s_mach.concurrent+ provides the +workers+ method for controlling the maximum allowed number of concurrent operations,
specified as the first parameter to the +workers+ method.
...
== Adding progress reporting, retry and Throttle control to concurrent collection operations
+s_mach.concurrent+ allows modifying collection concurrent operations to report operation progress, retry failures and
limit iteration speed to a specific time period.
