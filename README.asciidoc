...
* Adds new concurrent flow control primitives +concurrently+, +serially+ and +workers+ for performing collection operations such as +map+, +flatMap+ and +foreach+ when the called function returns a Future of the result (A => Future[B]) instead of the result (A => B)
