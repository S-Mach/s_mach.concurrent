...
object WorkersOps {
  ...
  def mapWorkers[A,B,M[+AA] <: TraversableOnce[AA]](
    ...
    runWorkers(self, f, output, workersConfig).map { _ => builder.result() }
  }
  ...
  def foreachWorkers[A,U,M[+AA] <: TraversableOnce[AA]](
    ...
    runWorkers(self, f, output, workersConfig).map { _ => () }
  }
   ...
   * @param workersConfig specifies the number of workers
